(function() {
  var HttpServer, NodeApplication, RackApplication, connect, dirname, exists, fs, join, pause, sys, version, _ref;
  ...
  module.exports = HttpServer = (function() {
    ...
    function HttpServer(configuration) {
      ...
      this.findNodeApplication = __bind(this.findNodeApplication, this);
      ...
      HttpServer.__super__.constructor.call(this, [o(this.logRequest), o(this.annotateRequest), o(this.handlePowRequest), o(this.findApplicationRoot), o(this.handleStaticRequest), o(this.findRackApplication), o(this.findNodeApplication), o(this.handleApplicationRequest), x(this.handleErrorStartingApplication), o(this.handleFaviconRequest), o(this.handleApplicationNotFound), o(this.handleWelcomeRequest), o(this.handleRailsAppWithoutRackupFile), o(this.handleLocationNotFound)]);
      this.staticHandlers = {};
    ...
    HttpServer.prototype.findRackApplication = function(req, res, next) {
      ...
      return exists(join(root, "config.ru"), __bind(function(rackConfigExists) {
        ...
        if (rackConfigExists) {
          req.pow.application = (_ref2 = (_base = this.webApplications)[root]) != null ? _ref2 : _base[root] = new RackApplication(this.configuration, root);
        } else if (application = this.webApplications[root]) {
          delete this.webApplications[root];
          application.quit();
        }
        return next();
      }, this));
    };
    HttpServer.prototype.findNodeApplication = function(req, res, next) {
      var root;
      if (!(root = req.pow.root)) {
        return next();
      }
      return exists(join(root, "package.json"), __bind(function(nodePackageExists) {
        if (nodePackageExists) {
          new RackApplication(this.configuration, root);
        }
        return next();
      }, this));
    };
